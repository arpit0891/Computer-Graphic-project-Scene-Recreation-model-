...
Arpit Gurudutt Mishra ( 181370 ,CS56)

Ishika Goel ( 181356 ,CS56)

Vatsal Agarwal ( 181359 ,CS56)
...
For compilation command used: 

gcc projectcg.c -lX11 -lGL -lm -lGLU -lglut -g -Wall -O2 -o u
...
#define TWO\_PI  2.0 \* PI

#define RAD\_TO\_DEG  180.0 / PI
...
`   `GLfloat full\_shininess[]={110.0};

`   `//Material Properties

`   `glMaterialfv(GL\_FRONT,GL\_AMBIENT,ambient);

`   `glMaterialfv(GL\_FRONT,GL\_SPECULAR,specular);

`   `glMaterialfv(GL\_FRONT,GL\_DIFFUSE,diffuse);

`   `glMaterialfv(GL\_FRONT,GL\_SHININESS, full\_shininess);
...
`               `glRotatef(angle\*RAD\_TO\_DEG,0,0,-1);
...
`           `gluLookAt(-30.0,-20.0,15,0.0,0.0,2.0,0.0, 0.0,1.0);

`           `car();

`           `glPushMatrix();

`               `glRotatef(angle\*RAD\_TO\_DEG, 0.0,0.0,1.0);
...
`               `glRotatef(RAD\_TO\_DEG \* angle, 0.0, 0.0, 1.0);
...
`   `glLightfv(GL\_LIGHT0, GL\_AMBIENT, amb);

`   `glLightfv(GL\_LIGHT0, GL\_DIFFUSE, diff);

`   `glLightfv(GL\_LIGHT0, GL\_SPECULAR, spec);
...
`   `printf("\t\tScene Recreation model\n\t\tCar on a track simulation\n");

`   `printf("\n\tPRESS:\n");

`   `printf("\n\tg:To Start or Continue\n");
